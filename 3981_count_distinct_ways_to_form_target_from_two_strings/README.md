...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/count-distinct-ways-to-form-target-from-two-strings/](https://leetcode.com/problems/count-distinct-ways-to-form-target-from-two-strings/)
...
1. Use dynamic programming over the current position in `target` and the next available indices in `word1` and `word2`.
2. For each character of `target`, try choosing a matching character from `word1` or from `word2`, then move only the corresponding string's index forward.
3. Track whether at least one character has been chosen from each string. This can be represented using a bitmask.
4. The final answer is the number of ways where all characters of `target` have been formed and both strings have been used.
