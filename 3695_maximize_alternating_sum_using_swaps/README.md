...
You are also given a 2D integer array `swaps` where `swaps[i] = [p_{i}, q_{i}]`. For each pair `[p_{i}, q_{i}]` in `swaps`, you are allowed to swap the elements at indices `p_{i}` and `q_{i}`. These swaps can be performed any number of times and in any order.
