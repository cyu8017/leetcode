...
You are given two integers `m` and `n` representing a **0-indexed** `m x n` grid. You are also given two 2D integer arrays `guards` and `walls` where `guards[i] = [row_{i}, col_{i}]` and `walls[j] = [row_{j}, col_{j}]` represent the positions of the `i^{th}` guard and `j^{th}` wall respectively.
