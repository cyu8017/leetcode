...
1. Brute-force all subsets for `B`.
2. Let `s` = XOR of all elements not in `B`.
3. We want to choose a value `x` (a subset‐XOR of the "remaining" elements) to maximize `x + (s XOR x)`.
4. Observe that `x + (s XOR x) = s + 2 * (x AND ~s)`.
5. To do this efficiently, build a linear XOR basis over the values `nums[j] & ~s` for each index `j` not in `B`.
6. Finally, greedily extract the maximum `x` from that basis.
