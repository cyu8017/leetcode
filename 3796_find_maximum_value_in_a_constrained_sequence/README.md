...
2. Any restriction at index `i` also indirectly limits nearby positions due to the cumulative `diff` bounds.
3. The maximum possible value at each index is the minimum of all bounds propagated from the left and from the right.
4. Once all upper bounds are fixed, the answer is simply the maximum value among them.
