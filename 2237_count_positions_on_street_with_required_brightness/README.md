...
You are given an integer `n`. A perfectly straight street is represented by a number line ranging from `0` to `n - 1`. You are given a 2D integer array `lights` representing the street lamp(s) on the street. Each `lights[i] = [position_{i}, range_{i}]` indicates that there is a street lamp at position `position_{i}` that lights up the area from `[max(0, position_{i} - range_{i}), min(n - 1, position_{i} + range_{i})]` (**inclusive**).
...
1. How can we find the brightness at every position on the street?
2. We can use a hash table to store the change in brightness from the previous position to the current position.
