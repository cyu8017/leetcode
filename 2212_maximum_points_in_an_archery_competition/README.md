...
- The target has integer scoring sections ranging from `0` to `11` **inclusive**.
		...
		- For **each** section of the target with score `k` (in between `0` to `11`), say Alice and Bob have shot `a_{k}` and `b_{k}` arrows on that section respectively. If `a_{k} >= b_{k}`, then Alice takes `k` points. If `a_{k} < b_{k}`, then Bob takes `k` points.
