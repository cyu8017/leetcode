...
2. For a given `x`, you need to check if `x` is the median, to the left of the median, or to the right of the median. You can do that by counting the number of sub-arrays `nums[i…j]` such that `distinct(num[i…j]) <= x`.
