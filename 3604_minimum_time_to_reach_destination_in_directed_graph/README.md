...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/minimum-time-to-reach-destination-in-directed-graph/](https://leetcode.com/problems/minimum-time-to-reach-destination-in-directed-graph/)
- **Tags:** graph-theory, heap-(priority-queue), shortest-path

## Problem

You are given an integer `n` and a **directed** graph with `n` nodes labeled from 0 to `n - 1`. This is represented by a 2D array `edges`, where `edges[i] = [u_{i}, v_{i}, start_{i}, end_{i}]` indicates an edge from node `u_{i}` to `v_{i}` that can **only** be used at any integer time `t` such that `start_{i} <= t <= end_{i}`.
...
- Wait at your current node without moving, or
	...
	- Travel along an outgoing edge from your current node if the current time `t` satisfies `start_{i} <= t <= end_{i}`.
...
- `1 <= n <= 10^{5}`
	...
	- `edges[i] == [u_{i}, v_{i}, start_{i}, end_{i}]`
...
2. At node `u` with current time `t`, you can only use an edge `[u, v, start, end]` if `t <= end`.
3. If `t < start`, wait until `start`, then traverse (arriving at `start + 1`).
