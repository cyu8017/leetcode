...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/integers-with-multiple-sum-of-two-cubes/](https://leetcode.com/problems/integers-with-multiple-sum-of-two-cubes/)
...
1. Iterate over all pairs `(a, b)` with `a <= b` and `a^{3} + b^{3} <= n`
2. Use a map to count how many times each value of `a^{3} + b^{3}` appears
