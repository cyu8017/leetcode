...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/minimize-the-total-price-of-the-trips/](https://leetcode.com/problems/minimize-the-total-price-of-the-trips/)
- **Tags:** array, dynamic-programming, tree, depth-first-search, graph-theory

## Problem

There exists an undirected and unrooted tree with `n` nodes indexed from `0` to `n - 1`. You are given the integer `n` and a 2D integer array `edges` of length `n - 1`, where `edges[i] = [a_{i}, b_{i}]` indicates that there is an edge between nodes `a_{i}` and `b_{i}` in the tree.
...
Additionally, you are given a 2D integer array `trips`, where `trips[i] = [start_{i}, end_{i}]` indicates that you start the `i^{th}` trip from the node `start_{i}` and travel to the node `end_{i}` by any path you like.
