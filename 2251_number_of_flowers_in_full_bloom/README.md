...
You are given a **0-indexed** 2D integer array `flowers`, where `flowers[i] = [start_{i}, end_{i}]` means the `i^{th}` flower will be in **full bloom** from `start_{i}` to `end_{i}` (**inclusive**). You are also given a **0-indexed** integer array `people` of size `n`, where `people[i]` is the time that the `i^{th}` person will arrive to see the flowers.
