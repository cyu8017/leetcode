...
All the friends are divided into pairs. The pairings are given in a list `pairs`, where `pairs[i] = [x_{i}, y_{i}]` denotes `x_{i}` is paired with `y_{i}` and `y_{i}` is paired with `x_{i}`.
