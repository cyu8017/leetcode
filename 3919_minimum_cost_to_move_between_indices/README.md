...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/minimum-cost-to-move-between-indices/](https://leetcode.com/problems/minimum-cost-to-move-between-indices/)
...
You are also given a 2D integer array `queries`, where each `queries[i] = [l_{i}, r_{i}]`.
...
1. Following `closest(x)` forms directed chains. If the chain moves toward the target index, you can consider walking along it with cost 1 per step.
2. Precompute prefix sums of these costs in both directions.
3. For each query, compare the cost of walking along closest edges vs making a direct jump.
