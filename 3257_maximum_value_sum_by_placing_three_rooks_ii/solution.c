...
typedef struct { int v, c; } Cell3257;

long long maximumValueSum(int** board, int boardSize, int* boardColSize) {
    int m = boardSize, n = boardColSize[0];
    Cell3257** tops = (Cell3257**)malloc((size_t)m * sizeof(Cell3257*));
    int* tlen = (int*)calloc((size_t)m, sizeof(int));
    for (int i = 0; i < m; i++) {
        tops[i] = (Cell3257*)malloc(3 * sizeof(Cell3257));
        tlen[i] = 0;
        for (int j = 0; j < n; j++) {
            Cell3257 cur = {board[i][j], j};
            int placed = 0;
    ...
    for (int i = 0; i < m; i++) {
        for (int ai = 0; ai < tlen[i]; ai++) {
            Cell3257 a = tops[i][ai];
            for (int j = i + 1; j < m; j++) {
                for (int bi = 0; bi < tlen[j]; bi++) {
                    Cell3257 b = tops[j][bi];
                    if (a.c == b.c) continue;
                    for (int k = j + 1; k < m; k++) {
                        for (int ci = 0; ci < tlen[k]; ci++) {
                            Cell3257 c = tops[k][ci];
