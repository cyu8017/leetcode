...
You are given an integer array `nums` of length `n` and a 2D array `queries`, where `queries[i] = [l_{i}, r_{i}, val_{i}]`.

Each `queries[i]` represents the following action on `nums`:

- Select a subset of indices in the range `[l_{i}, r_{i}]` from `nums`.

	- Decrement the value at each selected index by **exactly** `val_{i}`.
