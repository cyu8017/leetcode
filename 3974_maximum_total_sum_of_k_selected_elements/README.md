...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/maximum-total-sum-of-k-selected-elements/](https://leetcode.com/problems/maximum-total-sum-of-k-selected-elements/)
...
1. Suppose the selected elements are fixed. Which elements should be multiplied, and in what order should they be processed?
2. Since every `nums[i]` is positive, multiplying by a current value greater than 1 is always at least as good as adding normally.
3. Only the first `min(k, mul - 1)` processed elements should be multiplied. All remaining selected elements should be added normally.
4. Choose the largest `k` elements of `nums`, and pair the largest selected elements with the largest multipliers.
