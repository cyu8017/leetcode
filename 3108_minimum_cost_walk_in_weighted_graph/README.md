...
You are given the integer `n` and an array `edges`, where `edges[i] = [u_{i}, v_{i}, w_{i}]` indicates that there is an edge between vertices `u_{i}` and `v_{i}` with a weight of `w_{i}`.
...
You are also given a 2D array `query`, where `query[i] = [s_{i}, t_{i}]`. For each query, you need to find the minimum cost of the walk starting at vertex `s_{i}` and ending at vertex `t_{i}`. If there exists no such walk, the answer is `-1`.
