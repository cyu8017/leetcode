...
You are given two integers `n` and `k`, and a 2D integer array `invocations`, where `invocations[i] = [a_{i}, b_{i}]` indicates that method `a_{i}` invokes method `b_{i}`.
