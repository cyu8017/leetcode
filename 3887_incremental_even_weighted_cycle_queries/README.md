...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/incremental-even-weighted-cycle-queries/](https://leetcode.com/problems/incremental-even-weighted-cycle-queries/)
...
You are also given a 2D integer array `edges`, where `edges[i] = [u_{i}, v_{i}, w_{i}]` represents an edge between nodes `u_{i}` and `v_{i}` with weight `w_{i}`. The weight `w_{i}` is either 0 or 1.
