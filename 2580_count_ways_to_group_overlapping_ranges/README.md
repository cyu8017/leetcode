...
You are given a 2D integer array `ranges` where `ranges[i] = [start_{i}, end_{i}]` denotes that all integers between `start_{i}` and `end_{i}` (both **inclusive**) are contained in the `i^{th}` range.
