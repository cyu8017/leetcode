...
There are some robots and factories on the X-axis. You are given an integer array `robot` where `robot[i]` is the position of the `i^{th}` robot. You are also given a 2D integer array `factory` where `factory[j] = [position_{j}, limit_{j}]` indicates that `position_{j}` is the position of the `j^{th}` factory and that the `j^{th}` factory can repair at most `limit_{j}` robots.
...
- `1 <= robot.length, factory.length <= 100`
	...
	- `-10^{9} <= robot[i], position_{j} <= 10^{9}`

	- `0 <= limit_{j} <= robot.length`
