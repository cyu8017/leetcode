...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/minimum-threshold-path-with-limited-heavy-edges/](https://leetcode.com/problems/minimum-threshold-path-with-limited-heavy-edges/)
...
The graph is represented by a 2D integer array `edges`, where each edge `edges[i] = [u_{i}, v_{i}, w_{​​​​​​​i}]` indicates that there is an undirected edge between nodes `u_{i}` and `v_{i}` with weight `w_{​​​​​​​i}`.
