...
You are given a positive integer `n` which is the number of nodes of a **0-indexed undirected weighted connected** graph and a **0-indexed** **2D array** `edges` where `edges[i] = [u_{i}, v_{i}, w_{i}]` indicates that there is an edge between nodes `u_{i}` and `v_{i}` with weight `w_{i}`.
