...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/valid-subarrays-with-matching-sum-digits-ii/](https://leetcode.com/problems/valid-subarrays-with-matching-sum-digits-ii/)
- **Premium:** Yes
...
1. Since all elements are positive, prefix sums are strictly increasing.
2. A sum has first digit `x` iff it belongs to one of the ranges `[x * 10^{p}, (x + 1) * 10^{p} - 1]`.
3. Also require the sum to be congruent to `x` modulo 10, then count prefix-sum pairs whose difference lies in each valid range and residue class.
