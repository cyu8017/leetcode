...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/count-elements-with-at-least-k-greater-values/](https://leetcode.com/problems/count-elements-with-at-least-k-greater-values/)
...
1. Sort `nums`, build distinct `values` and `count`.
2. For each `val`: find `upper_bound`, compute `greater = n - upper_bound_idx`; if `greater >= k` add `count[val]` to `ans`.
