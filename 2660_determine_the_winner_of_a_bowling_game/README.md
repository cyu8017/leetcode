...
Assume a player hits `x_{i}` pins in the i^{th} turn. The value of the i^{th} turn for the player is:

- `2x_{i}` if the player hits 10 pins **in either (i - 1)^{th} or (i - 2)^{th} turn**.
