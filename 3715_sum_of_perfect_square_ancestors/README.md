...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/sum-of-perfect-square-ancestors/](https://leetcode.com/problems/sum-of-perfect-square-ancestors/)
- **Tags:** array, hash-table, math, tree, depth-first-search, counting, number-theory

## Problem

You are given an integer `n` and an undirected tree rooted at node 0 with `n` nodes numbered from 0 to `n - 1`. This is represented by a 2D array `edges` of length `n - 1`, where `edges[i] = [u_{i}, v_{i}]` indicates an undirected edge between nodes `u_{i}` and `v_{i}`.

You are also given an integer array `nums`, where `nums[i]` is the positive integer assigned to node `i`.

Define a value `t_{i}` as the number of **ancestors** of node `i` such that the product `nums[i] * nums[ancestor]` is a **perfect square**.
