...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/best-reachable-tower/](https://leetcode.com/problems/best-reachable-tower/)
- **Tags:** array

## Problem

You are given a 2D integer array `towers`, where `towers[i] = [x_{i}, y_{i}, q_{i}]` represents the coordinates `(x_{i}, y_{i})` and quality factor `q_{i}` of the `i^{th}` tower.
...
- `1 <= towers.length <= 10^{5}`
	...
	- `towers[i] = [x_{i}, y_{i}, q_{i}]`

	- `center = [cx, cy]`

	- `0 <= x_{i}, y_{i}, q_{i}, cx, cy <= 10^{5}`​​​​​​​
