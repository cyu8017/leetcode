...
**Output:** [2,3]
**Explanation:**
On day 2, we have security[0] >= security[1] >= security[2] = security[2] >= security[3] <= security[4] <= security[5].
...
3. Use an array to store the number of days before the i^{th} day that is non-increasing, and another array to store the number of days after the i^{th} day that is non-decreasing.
