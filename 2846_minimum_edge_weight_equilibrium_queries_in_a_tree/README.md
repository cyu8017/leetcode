...
There is an undirected tree with `n` nodes labeled from `0` to `n - 1`. You are given the integer `n` and a 2D integer array `edges` of length `n - 1`, where `edges[i] = [u_{i}, v_{i}, w_{i}]` indicates that there is an edge between nodes `u_{i}` and `v_{i}` with weight `w_{i}` in the tree.

You are also given a 2D integer array `queries` of length `m`, where `queries[i] = [a_{i}, b_{i}]`. For each query, find the **minimum number of operations** required to make the weight of every edge on the path from `a_{i}` to `b_{i}` equal. In one operation, you can choose any edge of the tree and change its weight to any value.
...
For each queries[i], it can be shown that answer[i] is the minimum number of operations needed to equalize all the edge weights in the path from a_{i} to b_{i}.
```
