...
You are also given a **0-indexed** 2D integer array `queries` where `queries[i] = [k_{i}, trim_{i}]`. For each `queries[i]`, you need to:

- **Trim** each number in `nums` to its **rightmost** `trim_{i}` digits.

	- Determine the **index** of the `k_{i}^{th}` smallest trimmed number in `nums`. If two trimmed numbers are equal, the number with the **lower** index is considered to be smaller.
