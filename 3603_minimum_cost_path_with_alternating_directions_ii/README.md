...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/minimum-cost-path-with-alternating-directions-ii/](https://leetcode.com/problems/minimum-cost-path-with-alternating-directions-ii/)
...
2. Observe that you need to wait at each cell except the first and last
3. Transition: `dp[i][j]` <- min(`dp[i‑1][j]`, `dp[i][j‑1]`) + `waitCost[i][j]` + (`i+1`)*(`j+1`)
4. The answer is `dp[m‑1][n‑1]` - `waitCost[m‑1][n‑1]`
