...
You are also given a 2D integer array `roads`, where `roads[i] = [u_{i}, v_{i}, cost_{i}, tax_{i}]` represents a **bidirectional** road:

- `u_{i}` and `v_{i}` are the shops connected by the road.

	- `cost_{i}` is the cost to travel the road **without** carrying apples.

	- `tax_{i}` is the multiplier applied to `cost_{i}` when traveling **with** apples.
...
- `1 <= n <= 1000`
	...
	- `roads[i] = [u_{i}, v_{i}, cost_{i}, tax_{i}]`
