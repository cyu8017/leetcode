...
You are given a **0-indexed** 2D integer array `tires` where `tires[i] = [f_{i}, r_{i}]` indicates that the `i^{th}` tire can finish its `x^{th}` successive lap in `f_{i} * r_{i}^{(x-1)}` seconds.
