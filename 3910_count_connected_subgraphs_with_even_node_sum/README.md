...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/count-connected-subgraphs-with-even-node-sum/](https://leetcode.com/problems/count-connected-subgraphs-with-even-node-sum/)
- **Tags:** array, bit-manipulation, depth-first-search, breadth-first-search, union-find, graph-theory, enumeration

## Problem

You are given an undirected graph with `n` nodes labeled from 0 to `n - 1`. Node `i` has a **value** of `nums[i]`, which is either 0 or 1. The edges of the graph are given by a 2D array `edges` where `edges[i] = [u_{i}, v_{i}]` represents an edge between node `u_{i}` and node `v_{i}`.
...
5. Since `n <= 13`, a brute-force bitmask solution is feasible
