...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/maximum-number-of-items-from-sale-i/](https://leetcode.com/problems/maximum-number-of-items-from-sale-i/)
...
- For each item `i` that you bought **at least one copy** of, you receive **one free copy** of every item `j` such that `j != i` and `factor_{i}` divides `factor_{j}`.
...
- You can buy 2 copies of item 0 and 1 copy of item 2 for a total cost of `2 * 2 + 4 = 8`, which is not greater than `budget = 9`.
	...
	- Buying item 2 gives 1 free copy of item 0, because `factor_{2} = 3` divides `factor_{0} = 6`.
