...
2. The operation only allows `'1'`s to move backward
3. Going from left to right, maintain a priority queue (max heap)
4. When at a `'1'`, pop the top of the priority queue if it is not empty
