...
You are given a **0-indexed** 2D integer array `brackets` where `brackets[i] = [upper_{i}, percent_{i}]` means that the `i^{th}` tax bracket has an upper bound of `upper_{i}` and is taxed at a rate of `percent_{i}`. The brackets are **sorted** by upper bound (i.e. `upper_{i-1} < upper_{i}` for `0 < i < brackets.length`).

Tax is calculated as follows:

- The first `upper_{0}` dollars earned are taxed at a rate of `percent_{0}`.

	- The next `upper_{1} - upper_{0}` dollars earned are taxed at a rate of `percent_{1}`.

	- The next `upper_{2} - upper_{1}` dollars earned are taxed at a rate of `percent_{2}`.
...
- `1 <= brackets.length <= 100`
	...
	- `upper_{i}` is sorted in ascending order.

	- All the values of `upper_{i}` are **unique**.
