...
Each query `queries[i] = [l_{i}, r_{i}]` asks us to find the number of strings present at the indices ranging from `l_{i}` to `r_{i}` (both **inclusive**) of `words` that start and end with a vowel.
