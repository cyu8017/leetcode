...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/maximum-requests-without-violating-the-limit/](https://leetcode.com/problems/maximum-requests-without-violating-the-limit/)
- **Premium:** Yes
- **Tags:** array, hash-table, greedy, sliding-window, sorting

## Problem

You are given a 2D integer array `requests`, where `requests[i] = [user_{i}, time_{i}]` indicates that `user_{i}` made a request at `time_{i}`.
...
**Constraints:**
	...
	- `requests[i] = [user_{i}, time_{i}]`

	- `1 <= k <= requests.length`

	- `1 <= user_{i}, time_{i}, window <= 10^{5}`
