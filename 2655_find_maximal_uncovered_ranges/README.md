...
By all of the **uncovered** ranges with **maximal** length, we mean satisfying two conditions:
	...
	- There should **not exist** two ranges (l_{1}, r_{1}) and (l_{2}, r_{2}) such that r_{1 }+ 1 = l_{2}
...
1. The complexity of the solution is independent of the length of the array nums.
2. Sort ranges by their start points.
3. An uncovered range should start right after the end of one of the input ranges (or starts at zero) and also ends right before the start of one of the input ranges (or ends at n).
