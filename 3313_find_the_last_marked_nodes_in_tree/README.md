...
There exists an **undirected** tree with `n` nodes numbered `0` to `n - 1`. You are given a 2D integer array `edges` of length `n - 1`, where `edges[i] = [u_{i}, v_{i}]` indicates that there is an edge between nodes `u_{i}` and `v_{i}` in the tree.
...
1. We need to calculate the height of tree when rooted at each node.
2. Find a diameter using two DFS.
3. The farthest node from each node must be one of the endpoints of the diameter.
4. The last marked node will be one of the end of the diameter, whichever is more distant.
