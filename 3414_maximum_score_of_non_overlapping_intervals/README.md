...
You are given a 2D integer array `intervals`, where `intervals[i] = [l_{i}, r_{i}, weight_{i}]`. Interval `i` starts at position `l_{i}` and ends at `r_{i}`, and has a weight of `weight_{i}`. You can choose *up to* 4 **non-overlapping** intervals. The **score** of the chosen intervals is defined as the total sum of their weights.
...
- `1 <= intevals.length <= 5 * 10^{4}`
	...
	- `intervals[i] = [l_{i}, r_{i}, weight_{i}]`
