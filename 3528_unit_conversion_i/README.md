...
There are `n` types of units indexed from `0` to `n - 1`. You are given a 2D integer array `conversions` of length `n - 1`, where `conversions[i] = [sourceUnit_{i}, targetUnit_{i}, conversionFactor_{i}]`. This indicates that a single unit of type `sourceUnit_{i}` is equivalent to `conversionFactor_{i}` units of type `targetUnit_{i}`.
