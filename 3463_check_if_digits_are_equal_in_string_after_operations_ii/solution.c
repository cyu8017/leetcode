...
static int modPowP(int a, int e, int p) {
    int r = 1;
    while (e > 0) {
        if (e & 1) r = r * a % p;
        a = a * a % p;
...
static int modInvPrime(int a, int p) { return modPowP(a, p - 2, p); }

static int binomMod(int n, int k, int p) {
    if (k < 0 || k > n) return 0;
    int num = 1, den = 1;
    for (int i = 0; i < k; i++) {
        num = num * (n - i) % p;
...
static int crt(int a1, int m1, int a2, int m2) {
...
static int binomMod10(int n, int k) {
    return crt(binomMod(n, k, 2), 2, binomMod(n, k, 5), 5);
}

static int combineDigit(char* s, int n, int offset) {
    int sum = 0;
    for (int i = 0; i <= n - 2; i++) {
