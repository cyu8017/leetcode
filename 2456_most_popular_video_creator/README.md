...
Return* *a **2D array** of **strings** `answer` where `answer[i] = [creators_{i}, id_{i}]` means that `creators_{i}` has the **highest** popularity and `id_{i}` is the **id** of their most **popular** video. The answer can be returned in any order.
