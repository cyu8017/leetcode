...
Each video clip is described by an array `clips` where `clips[i] = [start_{i}, end_{i}]` indicates that the ith clip started at `start_{i}` and ended at `end_{i}`.
