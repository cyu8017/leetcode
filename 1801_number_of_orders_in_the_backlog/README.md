...
You are given a 2D integer array `orders`, where each `orders[i] = [price_{i}, amount_{i}, orderType_{i}]` denotes that `amount_{i}`_{ }orders have been placed of type `orderType_{i}` at the price `price_{i}`. The `orderType_{i}` is:

- `0` if it is a batch of `buy` orders, or

	- `1` if it is a batch of `sell` orders.

Note that `orders[i]` represents a batch of `amount_{i}` independent orders with the same price and order type. All orders represented by `orders[i]` will be placed before all orders represented by `orders[i+1]` for all valid `i`.
