...
function getNumberOfBacklogOrders(orders) {
    ...
    const popBuy = () => {
        ...
        while (true) {
            ...
            if (l < n && buy[l][0] < buy[s][0])
                s = l;
            if (r < n && buy[r][0] < buy[s][0])
                s = r;
            if (s === i)
                break;
    ...
    const popSell = () => {
        ...
        while (true) {
            let s = i;
            const l = 2 * i + 1, r = 2 * i + 2;
            if (l < n && sell[l][0] < sell[s][0])
                s = l;
            if (r < n && sell[r][0] < sell[s][0])
                s = r;
            if (s === i)
                break;
    ...
    for (const [price, amount, orderType] of orders) {
        ...
        while (buy.length && sell.length && -buy[0][0] >= sell[0][0]) {
            ...
            sellAmount -= matched;
            if (buyAmount)
                pushBuy(-negBuyPrice, buyAmount);
            if (sellAmount)
                pushSell(sellPrice, sellAmount);
        }
    }
    let total = 0;
    for (const [, amount] of buy)
        total = (total + amount) % MOD;
    for (const [, amount] of sell)
        total = (total + amount) % MOD;
    return total;
}
