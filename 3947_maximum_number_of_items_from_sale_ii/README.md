...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/maximum-number-of-items-from-sale-ii/](https://leetcode.com/problems/maximum-number-of-items-from-sale-ii/)
- **Tags:** array, greedy, sorting, heap-(priority-queue)

## Problem

You are given a 2D integer array `items`, where `items[i] = [factor_{i}, price_{i}]` represents the `i^{th}` item. You are also given an integer `budget`.
...
- Each purchased copy of item `i` can give you **at most one** free copy of another item `j`.
	...
	- The free item must satisfy `i != j` and `factor_{i}` divides `factor_{j}`.
...
- You can buy 2 copies of item 0 and 1 copy of item 1 for a total cost of `2 * 6 + 4 = 16`, which is not greater than `budget = 19`.
	...
	- One purchased copy of item 0 gives 1 free copy of item 1, because `factor_{0} = 1` divides `factor_{1} = 2`.

	- The other purchased copy of item 0 gives 1 free copy of item 2, because `factor_{0} = 1` divides `factor_{2} = 3`.
...
- You can buy 2 copies of item 0, 1 copy of item 1, and 1 copy of item 2 for a total cost of `2 * 8 + 10 + 6 = 32`, which is not greater than `budget = 35`.
	...
	- One purchased copy of item 0 gives 1 free copy of item 2, because `factor_{0} = 2` divides `factor_{2} = 6`.

	- The other purchased copy of item 0 gives 1 free copy of item 3, because `factor_{0} = 2` divides `factor_{3} = 4`.

	- The purchased copy of item 1 gives 1 free copy of item 2, because `factor_{1} = 1` divides `factor_{2} = 6`.

	- Buying item 2 gives no free copy, because `factor_{2} = 6` does not divide the factor of any other item.
...
- `1 <= items.length <= 10^{5}`
	...
	- `items[i] = [factor_{i}, price_{i}]`

	- `1 <= factor_{i} <= items.length`
