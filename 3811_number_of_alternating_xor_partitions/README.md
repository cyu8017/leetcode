...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/number-of-alternating-xor-partitions/](https://leetcode.com/problems/number-of-alternating-xor-partitions/)
...
2. Compute prefix XORs so the XOR of a block `[l..r]` can be obtained as `pref[r] ^ pref[l - 1]`.
3. For each position, try ending a block whose XOR equals the expected target, and transition to the next state (alternating between `target1` and `target2`).
4. Use a hash map or array indexed by prefix XOR values to efficiently count valid previous split points.
