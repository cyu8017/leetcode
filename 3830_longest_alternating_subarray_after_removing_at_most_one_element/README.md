...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/longest-alternating-subarray-after-removing-at-most-one-element/](https://leetcode.com/problems/longest-alternating-subarray-after-removing-at-most-one-element/)
...
- Choose not to remove elements.
	...
	- Select the entire array `[**2, 1, 3, 2**]`, which is alternating because `2 > 1  2`.
...
3. Try removing each index `r`: if `nums[r - 1] < nums[r + 1]`, the two sides can connect with pattern ``, giving length `left[r - 1][0] + right[r + 1][1]`; if `nums[r - 1] > nums[r + 1]`, connect with pattern `> <`, giving `left[r - 1][1] + right[r + 1][0]`.
