...
You are given a 2D integer matrix `queries`, where for `queries[i] = [from_{i}, to_{i}, mod_{i}]` you should calculate `(big_nums[from_{i}] * big_nums[from_{i} + 1] * ... * big_nums[to_{i}]) % mod_{i}`.
...
1. Find a way to calculate `f(n, i)` which is the total number of numbers in `[1, n]` when the `i^{th}` bit is set in `O(log(n))` time.
