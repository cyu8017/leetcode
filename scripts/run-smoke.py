#!/usr/bin/env python3
"""Run smoke tests from config/smoke-problems.json."""
...
def load_manifest(repo_root: Path) -> list[dict]:
    manifest_path = repo_root / "config" / "smoke-problems.json"
...
def main() -> int:
    ...
    if not entries:
        print("No smoke entries configured.", file=sys.stderr)
        return 2
    ...
    for entry in entries:
        folder = entry["folder"]
        languages = entry.get("languages") or ["python"]
