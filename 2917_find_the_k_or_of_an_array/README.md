...
Only bits 0 and 3 qualify. The result is `(1001)_{2} = 9`.
...
1. Fix a `bit` from the range `[0, 31]`, then count the number of elements of `nums` that have `bit` set in them.
2. `bit` is set in integer `x` if and only if `2^{bit} AND x == 2^{bit}`, where `AND` is the bitwise `AND` operation.
