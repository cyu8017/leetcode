...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/minimum-energy-to-maintain-brightness/](https://leetcode.com/problems/minimum-energy-to-maintain-brightness/)
- **Tags:** array, sorting
...
You are also given an integer `brightness` and a 2D integer array `intervals`, where `intervals[i] = [start_{i}, end_{i}]` represents an **inclusive** time interval during which the lighting requirement **must** be satisfied.
...
1. The actual bulb configuration does not depend on the time unit, only on the required brightness.
2. First compute the minimum number of bulbs needed to illuminate at least brightness positions on a line of length `n`.
3. One bulb can illuminate at most 3 positions, except near the boundaries. So the answer is essentially `ceil(brightness / 3)`.
4. Merge all overlapping intervals to compute the total number of active time units.
