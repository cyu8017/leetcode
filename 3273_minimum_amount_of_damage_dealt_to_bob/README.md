...
2. For any two enemies `i` and `j` with damages `damage[i]` and `damage[j]`, and time to take each of them down `t_{i}` and `t_{j}`, when is it better to choose enemy `i` over enemy `j` first?
