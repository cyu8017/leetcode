...
A sequence `x_{1}, x_{2}, ..., x_{n}` is *Fibonacci-like* if:

- `n >= 3`

	- `x_{i} + x_{i+1} == x_{i+2}` for all `i + 2 <= n`
...
2. Consider a sequence ending at index `iarr[i]`, which can be done in linear time per element.
