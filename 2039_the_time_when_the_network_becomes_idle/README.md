...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/the-time-when-the-network-becomes-idle/](https://leetcode.com/problems/the-time-when-the-network-becomes-idle/)
- **Tags:** array, breadth-first-search, graph-theory

## Problem

There is a network of `n` servers, labeled from `0` to `n - 1`. You are given a 2D integer array `edges`, where `edges[i] = [u_{i}, v_{i}]` indicates there is a message channel between servers `u_{i}` and `v_{i}`, and they can pass **any** number of messages to **each other** directly in **one** second. You are also given a **0-indexed** integer array `patience` of length `n`.
