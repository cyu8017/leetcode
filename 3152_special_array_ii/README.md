...
You are given an array of integer `nums` and a 2D integer matrix `queries`, where for `queries[i] = [from_{i}, to_{i}]` your task is to check that subarray `nums[from_{i}..to_{i}]` is **special** or not.

Return an array of booleans `answer` such that `answer[i]` is `true` if `nums[from_{i}..to_{i}]` is special.
