...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/longest-palindromic-path-in-graph/](https://leetcode.com/problems/longest-palindromic-path-in-graph/)
- **Tags:** string, dynamic-programming, bit-manipulation, graph-theory, bitmask

## Problem

You are given an integer `n` and an **undirected** graph with `n` nodes labeled from 0 to `n - 1` and a 2D array `edges`, where `edges[i] = [u_{i}, v_{i}]` indicates an edge between nodes `u_{i}` and `v_{i}`.
