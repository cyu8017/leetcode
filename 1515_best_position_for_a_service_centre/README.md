...
Given an array `positions` where `positions[i] = [x_{i}, y_{i}]` is the position of the `ith` customer on the map, return *the minimum sum of the euclidean distances* to all customers.
...
**Output:** 4.00000
**Explanation:** As shown, you can see that choosing [x_{centre}, y_{centre}] = [1, 1] will make the distance to each customer = 1, the sum of all distances is 4 which is the minimum possible we can achieve.
```
