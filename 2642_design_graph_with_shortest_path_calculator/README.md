...
There is a **directed weighted** graph that consists of `n` nodes numbered from `0` to `n - 1`. The edges of the graph are initially represented by the given array `edges` where `edges[i] = [from_{i}, to_{i}, edgeCost_{i}]` meaning that there is an edge from `from_{i}` to `to_{i}` with the cost `edgeCost_{i}`.
...
- `1 <= n <= 100`
	...
	- `0 <= from_{i}, to_{i}, from, to, node1, node2 <= n - 1`

	- `1 <= edgeCost_{i}, edgeCost <= 10^{6}`
