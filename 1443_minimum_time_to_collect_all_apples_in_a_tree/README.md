...
The edges of the undirected tree are given in the array `edges`, where `edges[i] = [a_{i}, b_{i}]` means that exists an edge connecting the vertices `a_{i}` and `b_{i}`. Additionally, there is a boolean array `hasApple`, where `hasApple[i] = true` means that vertex `i` has an apple; otherwise, it does not have any apple.
