...
1. If there is a subsequence of length `j` with the sum of elements `k`, it contributes `2^{n - j}` to the answer.
2. Let `dp[i][j]` represent the number of subsequences in the subarray `nums[0..i]` which have a sum of `j`.
3. We can find the `dp[i][k]` for all `0 <= i <= n-1` and multiply them with `2^{n - j}` to get final answer.
