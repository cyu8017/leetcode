...
Conversely, if you have `k` (`k >= 3`) **positive** real numbers `a_{1}`, `a_{2}`, `a_{3}`, ..., `a_{k}` where `a_{1} <= a_{2} <= a_{3} <= ... <= a_{k}` **and** `a_{1} + a_{2} + a_{3} + ... + a_{k-1} > a_{k}`, then there **always** exists a polygon with `k` sides whose lengths are `a_{1}`, `a_{2}`, `a_{3}`, ..., `a_{k}`.
