...
export function shortestDistance(grid: number[][]): number {
    ...
    for (let row = 0; row < rows; row += 1) {
        for (let col = 0; col < cols; col += 1) {
            ...
            while (queue.length > 0) {
                const [currentRow, currentCol, distance] = queue.shift() as [number, number, number];
    ...
    for (let row = 0; row < rows; row += 1) {
        for (let col = 0; col < cols; col += 1) {
            if (grid[row][col] === 0 && reach[row][col] === buildings) best = Math.min(best, distances[row][col]);
        }
    }
