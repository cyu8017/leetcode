...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/number-of-integers-with-popcount-depth-equal-to-k-ii/](https://leetcode.com/problems/number-of-integers-with-popcount-depth-equal-to-k-ii/)
...
- `p_{0} = x`
	...
	- `p_{i+1} = popcount(p_{i})` for all `i >= 0`, where `popcount(y)` is the number of set bits (1's) in the binary representation of `y`.

This sequence will eventually reach the value 1.

The **popcount-depth** of `x` is defined as the **smallest** integer `d >= 0` such that `p_{d} = 1`.
...
1. Precompute `depth[i]` for each `nums[i]` by applying popcount until you reach 1.
2. Maintain six Fenwick trees `fenw[0]` through `fenw[5]`, where `fenw[d]` stores a 1 at index `i` iff `depth[i] == d`.
3. For an update `[2, idx, val]`, remove index idx from its old `fenw[old_depth]` and insert into `fenw[new_depth]`; for a query `[1, l, r, k]`, return `fenw[k].query(r) - fenw[k].query(l-1)`.
