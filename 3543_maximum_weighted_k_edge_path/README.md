...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/maximum-weighted-k-edge-path/](https://leetcode.com/problems/maximum-weighted-k-edge-path/)
- **Tags:** hash-table, dynamic-programming, graph-theory

## Problem

You are given an integer `n` and a **Directed Acyclic Graph (DAG)** with `n` nodes labeled from 0 to `n - 1`. This is represented by a 2D array `edges`, where `edges[i] = [u_{i}, v_{i}, w_{i}]` indicates a directed edge from node `u_{i}` to `v_{i}` with weight `w_{i}`.
