...
You are given a 2D integer array `ranges` and two integers `left` and `right`. Each `ranges[i] = [start_{i}, end_{i}]` represents an **inclusive** interval between `start_{i}` and `end_{i}`.

Return `true` *if each integer in the inclusive range* `[left, right]` *is covered by **at least one** interval in* `ranges`. Return `false` *otherwise*.

An integer `x` is covered by an interval `ranges[i] = [start_{i}, end_{i}]` if `start_{i} <= x <= end_{i}`.
