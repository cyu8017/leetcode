...
export function maxSubstringLength(s: string): number {
    const first = new Array(26).fill(-1), last = new Array(26).fill(0);
    const n = s.length;
    for (let i = 0; i < n; i++) {
    ...
    for (let k = 0; k < 26; k++) {
        ...
        let mx = last[k];
        for (let j = i; j < n; j++) {
            const a = first[s.charCodeAt(j) - 97], b = last[s.charCodeAt(j) - 97];
            if (a < i) break;
            mx = Math.max(mx, b);
