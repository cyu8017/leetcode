...
There is a test that has `n` types of questions. You are given an integer `target` and a **0-indexed** 2D integer array `types` where `types[i] = [count_{i}, marks_{i}]` indicates that there are `count_{i}` questions of the `i^{th}` type, and each one of them is worth `marks_{i}` points.
