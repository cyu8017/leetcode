...
You are given a 2D integer array `adjacentPairs` of size `n - 1` where each `adjacentPairs[i] = [u_{i}, v_{i}]` indicates that the elements `u_{i}` and `v_{i}` are adjacent in `nums`.
