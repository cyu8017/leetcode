...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/count-subtrees-with-max-distance-between-cities/](https://leetcode.com/problems/count-subtrees-with-max-distance-between-cities/)
- **Tags:** dynamic-programming, bit-manipulation, tree, enumeration, bitmask

## Problem

There are `n` cities numbered from `1` to `n`. You are given an array `edges` of size `n-1`, where `edges[i] = [u_{i}, v_{i}]` represents a bidirectional edge between cities `u_{i}` and `v_{i}`. There exists a unique path between each pair of cities. In other words, the cities form a **tree**.
...
- `2 <= n <= 15`
	...
	- All pairs `(u_{i}, v_{i})` are distinct.
