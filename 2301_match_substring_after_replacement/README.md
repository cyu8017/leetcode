...
You are given two strings `s` and `sub`. You are also given a 2D character array `mappings` where `mappings[i] = [old_{i}, new_{i}]` indicates that you may perform the following operation **any** number of times:

- **Replace** a character `old_{i}` of `sub` with `new_{i}`.
...
- `1 <= sub.length <= s.length <= 5000`
	...
	- `old_{i}` and `new_{i}` are either uppercase or lowercase English letters or digits.
