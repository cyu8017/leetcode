...
2. For each player with id `x` and for every `i` in the range `[0, ceil(log_{2}k)]`, we can determine the last receiver's id and compute the sum of player ids who receive the ball after `2^{i}` passes, starting from `x`.
...
4. Then for `i` in range `[1, ceil(log_{2}k)]`, `last_receiver[x][i] = last_receiver[last_receiver[x][i - 1]][i - 1]` and `sum[x][i] = sum[x][i - 1] + sum[last_receiver[x][i - 1]][i - 1]`, for all `x` in the range `[0, n - 1]`.
