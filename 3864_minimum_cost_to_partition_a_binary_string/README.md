...
2. For a segment, let `cost_no_split = (X == 0 ? flatCost : L * X * encCost)`. If length is even, compute `cost_split = cost(left) + cost(right)` and take the minimum.
3. Precompute prefix sums of ones so `X` (number of '1's) for any segment is O(1).
