...
There is a school that has classes of students and each class will be having a final exam. You are given a 2D integer array `classes`, where `classes[i] = [pass_{i}, total_{i}]`. You know beforehand that in the `i^{th}` class, there are `total_{i}` total students, but only `pass_{i}` number of students will pass the exam.
