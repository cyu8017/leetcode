...
You are given an integer `n` and an undirected graph with `n` nodes labeled from 0 to `n - 1`. This is represented by a 2D array `edges`, where `edges[i] = [u_{i}, v_{i}, time_{i}]` indicates an undirected edge between nodes `u_{i}` and `v_{i}` that can be removed at `time_{i}`.
