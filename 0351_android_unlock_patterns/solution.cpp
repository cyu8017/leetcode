...
class Solution {
    int jumpMiddle(int last, int nextCell) {
    ...
    bool isValid(int visited, int last, int nextCell) {
        ...
        return std::abs(last / 3 - nextCell / 3) <= 1
            && std::abs(last % 3 - nextCell % 3) <= 1;
    }

    int dfs(int visited, int last, int length, int m, int n) {
        if (length > n) {
            return 0;
        }
