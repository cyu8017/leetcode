...
There are `n` cars traveling at different speeds in the same direction along a one-lane road. You are given an array `cars` of length `n`, where `cars[i] = [position_{i}, speed_{i}]` represents:

- `position_{i}` is the distance between the `i^{th}` car and the beginning of the road in meters. It is guaranteed that `position_{i} < position_{i+1}`.

	- `speed_{i}` is the initial speed of the `i^{th}` car in meters per second.
