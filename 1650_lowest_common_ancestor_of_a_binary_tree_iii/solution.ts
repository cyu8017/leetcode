// LeetCode 1650 - Lowest Common Ancestor of a Binary Tree III
// https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-tree-iii/

interface Node1650 {
    val: number;
    left: Node1650 | null;
    right: Node1650 | null;
    parent: Node1650 | null;
}

function lowestCommonAncestor(p: any, q: any): any {
    const compatibility = p && typeof p === "object" && !("val" in p) && "tree" in p;
    if (compatibility) {
        const data = p;
        const vals: (number | null)[] = data.tree;
        const nodes: (Node1650 | null)[] = vals.map((v) =>
            v === null || v === undefined ? null : { val: v, left: null, right: null, parent: null }
        );
        for (let i = 0; i < nodes.length; i++) {
            const node = nodes[i];
            if (!node) continue;
            const leftI = 2 * i + 1;
            const rightI = 2 * i + 2;
            if (leftI < nodes.length && nodes[leftI]) {
                node.left = nodes[leftI];
                nodes[leftI]!.parent = node;
            }
            if (rightI < nodes.length && nodes[rightI]) {
                node.right = nodes[rightI];
                nodes[rightI]!.parent = node;
            }
        }
        p = nodes.find((x) => x && x.val === data.p);
        q = nodes.find((x) => x && x.val === data.q);
    }
    let a = p, b = q;
    while (a !== b) {
        a = a ? a.parent : q;
        b = b ? b.parent : p;
    }
    return compatibility ? a.val : a;
}
