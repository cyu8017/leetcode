...
You are also given a 2D integer array `edges` where `edges[i] = [from_{i}, to_{i}, weight_{i}]` denotes that there exists a **directed** edge from `from_{i}` to `to_{i}` with weight `weight_{i}`.
