...
You are also given a 2D integer array `queries`. For each `queries[i] = [u_{i}, v_{i}]`, find the **minimum** distance between nodes `u_{i}` and `v_{i}`_{.} If no path exists between the two nodes, return -1 for that query.
