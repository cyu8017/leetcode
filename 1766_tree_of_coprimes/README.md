...
To represent this tree, you are given an integer array `nums` and a 2D array `edges`. Each `nums[i]` represents the `i^{th}` node's value, and each `edges[j] = [u_{j}, v_{j}]` represents an edge between nodes `u_{j}` and `v_{j}` in the tree.
