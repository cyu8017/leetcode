...
4. If we have a conflicting pair `(x, y)` with `x < y`: 1. Sort the conflicting pairs by `y` values in non-increasing order.  2. Update each prefix of the `f` array accordingly.
