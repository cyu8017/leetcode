...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/minimum-cost-to-merge-sorted-lists/](https://leetcode.com/problems/minimum-cost-to-merge-sorted-lists/)
...
3. Let `dp[mask]` represent the minimum cost to merge all lists in `mask`
4. `dp[mask] = min(dp[s] + dp[mask ^ s] + cost(s, mask ^ s))`, where `s` and `mask ^ s` are nonempty disjoint submasks whose union is `mask`
