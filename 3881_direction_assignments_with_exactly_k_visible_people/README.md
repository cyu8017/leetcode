...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/direction-assignments-with-exactly-k-visible-people/](https://leetcode.com/problems/direction-assignments-with-exactly-k-visible-people/)
...
1. Use combinatorics
2. Count the visible people on the left and right independently. Let `a = pos` and `b = n - pos - 1`.
3. To see exactly `k` people, choose `i` visible people from the left side and `k - i` from the right side.
4. Each chosen left person must pick `L`, each chosen right person must pick `R`, and every other person can pick either direction.
5. Sum over all valid `i`, multiplying the two binomial choices and the free choices for the remaining people.
