...
1. Greedily try to add the smallest possible number in the array `nums`, such that `nums` contains distinct positive integers, and there are no two indices `i` and `j` with `nums[i] + nums[j] == target`.
