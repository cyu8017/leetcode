...
class Solution {
    func boldWords(_ words: [String], _ s: String) -> String {
        ...
        while i < n {
            if bold[i] {
                parts += "<b>"
                while i < n && bold[i] { parts.append(chars[i]); i += 1 }
                parts += "</b>"
            } else {
                parts.append(chars[i]); i += 1
            }
        }
        return parts
    }
