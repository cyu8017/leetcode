...
You are given a 2D integer array `meetings` where `meetings[i] = [start_{i}, end_{i}]` means that a meeting will be held during the **half-closed** time interval `[start_{i}, end_{i})`. All the values of `start_{i}` are **unique**.
...
- `1 <= n <= 100`
	...
	- `0 <= start_{i} < end_{i} <= 5 * 10^{5}`

	- All the values of `start_{i}` are **unique**.
