...
You have to divide `nums` into `m` **disjoint contiguous** subarrays such that for the `i^{th}` subarray `[l_{i}, r_{i}]`, the bitwise `AND` of the subarray elements is equal to `andValues[i]`, in other words, `nums[l_{i}] & nums[l_{i} + 1] & ... & nums[r_{i}] == andValues[i]` for all `1 <= i <= m`, where `&` represents the bitwise `AND` operator.
