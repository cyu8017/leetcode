...
You are given an integer `n` and an undirected tree with `n` nodes numbered from 0 to `n - 1`. The tree is represented by a 2D array `edges` of length `n - 1`, where `edges[i] = [u_{i}, v_{i}]` indicates an undirected edge between `u_{i}` and `v_{i}`.
