...
- **Difficulty:** Easy
- **LeetCode:** [https://leetcode.com/problems/sum-of-compatible-numbers-in-range-i/](https://leetcode.com/problems/sum-of-compatible-numbers-in-range-i/)
...
1. The condition `abs(n - x) <= k` means `x` is in the range `[n - k, n + k]`.
2. Since `x` must be positive, start checking from `max(1, n - k)`.
3. Iterate through all values in this range and add `x` to the answer when `(n & x) == 0`.
