...
You are given a **0-indexed** integer array `scores` of length `n` where `scores[i]` denotes the score of node `i`. You are also given a 2D integer array `edges` where `edges[i] = [a_{i}, b_{i}]` denotes that there exists an **undirected** edge connecting nodes `a_{i}` and `b_{i}`.
