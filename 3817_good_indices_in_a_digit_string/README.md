...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/good-indices-in-a-digit-string/](https://leetcode.com/problems/good-indices-in-a-digit-string/)
- **Premium:** Yes
- **Tags:** math, string
...
1. For an index `i`, any valid substring must end exactly at position `i`.
2. The substring you are looking for is determined by the decimal digits of `i`.
3. For each index `i`, compare the string form of `i` with the substring that ends at `i` and has the same length.
