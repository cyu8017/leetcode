...
There are `n` cities numbered from `0` to `n-1`. Given the array `edges` where `edges[i] = [from_{i}, to_{i}, weight_{i}]` represents a bidirectional and weighted edge between cities `from_{i}` and `to_{i}`, and given the integer `distanceThreshold`.
...
- `2 <= n <= 100`
	...
	- All pairs `(from_{i}, to_{i})` are distinct.
