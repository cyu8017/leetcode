...
These stations are interconnected via n **bidirectional** cables, represented by a 2D array connections, where each element connections[i] = [u_{i}, v_{i}] indicates a connection between station `u_{i}` and station `v_{i}`. Stations that are directly or indirectly connected form a **power grid**.
