...
You are given a positive integer `n` representing `n` cities numbered from `1` to `n`. You are also given a **2D** array `roads`, where `roads[i] = [a_{i}, b_{i}, cost_{i}]` indicates that there is a **bidirectional **road between cities `a_{i}` and `b_{i}` with a cost of traveling equal to `cost_{i}`.
...
1. Notice that the path you take to buy an apple will be the same path to return to the starting city.
2. Starting from each city, use Dijkstra’s algorithm to find the best city to travel to.
