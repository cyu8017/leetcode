...
class Solution {
    fun isSolvable(words: Array<String>, result: String): Boolean {
        ...
        val chars = (words.joinToString("") + result).toSet()
        ...
        fun dfs(column: Int, row: Int, total: Int): Boolean {
            ...
            if (row < words.size) {
                ...
                if (ch in value) return dfs(column, row + 1, total + value[ch]!!)
                for (digit in 0..9) {
            ...
            val digit = total % 10
            val carry = total / 10
            if (ch in value) return value[ch] == digit && dfs(column + 1, 0, carry)
