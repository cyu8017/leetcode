...
- Otherwise, we need to find the rightmost index `r_{2}` such that the number of distinct characters in the range `[r:r_{2}]` is at most `k`. The answer in this case is `2 + pref[partition_start[i] - 1] + suff[r_{2} + 1]`
