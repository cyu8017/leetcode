...
Some of the grid squares are `obstacles`. The `i^{th}` obstacle is at grid point `obstacles[i] = (x_{i}, y_{i})`. If the robot runs into an obstacle, it will stay in its current location (on the block adjacent to the obstacle) and move onto the next command.
