...
There is a restaurant with a single chef. You are given an array `customers`, where `customers[i] = [arrival_{i}, time_{i}]:`

- `arrival_{i}` is the arrival time of the `i^{th}` customer. The arrival times are sorted in **non-decreasing** order.

	- `time_{i}` is the time needed to prepare the order of the `i^{th}` customer.
