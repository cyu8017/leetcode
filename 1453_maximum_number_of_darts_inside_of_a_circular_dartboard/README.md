...
Alice is throwing `n` darts on a very large wall. You are given an array `darts` where `darts[i] = [x_{i}, y_{i}]` is the position of the `i^{th}` dart that Alice threw on the wall.
