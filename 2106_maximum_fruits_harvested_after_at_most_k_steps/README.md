...
Fruits are available at some positions on an infinite x-axis. You are given a 2D integer array `fruits` where `fruits[i] = [position_{i}, amount_{i}]` depicts `amount_{i}` fruits at the position `position_{i}`. `fruits` is already **sorted** by `position_{i}` in **ascending order**, and each `position_{i}` is **unique**.
...
- `1 <= fruits.length <= 10^{5}`
	...
	- `0 <= startPos, position_{i} <= 2 * 10^{5}`

	- `position_{i-1} < position_{i}` for any `i > 0` (**0-indexed**)
