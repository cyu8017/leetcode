...
The bombs are represented by a **0-indexed** 2D integer array `bombs` where `bombs[i] = [x_{i}, y_{i}, r_{i}]`. `x_{i}` and `y_{i}` denote the X-coordinate and Y-coordinate of the location of the `i^{th}` bomb, whereas `r_{i}` denotes the **radius** of its range.
