...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/maximum-product-of-two-integers-with-no-common-bits/](https://leetcode.com/problems/maximum-product-of-two-integers-with-no-common-bits/)
...
2. Create an array `dp` of size `1 << B`, where `B` is your bit‑width.
...
5. For a number `x` with mask `mx`, compute its "complement mask" as `cm = ~mx & ((1 << B)-1)`.
