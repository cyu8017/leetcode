...
You are given a 2D integer array `stockPrices` where `stockPrices[i] = [day_{i}, price_{i}]` indicates the price of the stock on day `day_{i}` is `price_{i}`. A **line chart** is created from the array by plotting the points on an XY plane with the X-axis representing the day and the Y-axis representing the price and connecting adjacent points. One such example is shown below:
