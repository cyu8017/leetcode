...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/maximum-valid-pair-sum/](https://leetcode.com/problems/maximum-valid-pair-sum/)
...
1. For a fixed index `j`, the index `i` can be at most `j - k`.
2. Iterate `j` from `k` to `nums.length - 1`, and maintain the maximum value among `nums[0], nums[1], ..., nums[j - k]`.
3. Use this maintained maximum as the best possible first element for the pair ending at `j`.
