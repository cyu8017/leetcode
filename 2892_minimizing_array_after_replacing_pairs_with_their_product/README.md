...
5. For a fixed `i`, you could go backward from `i^{th}` elements and multiply them together until the product is at most `k`. Now if you are currently on index `j` and you've merged all elements from `j^{th}` element to `i^{th}` element, `dp[i] = min(dp[i], dp[j - 1] + 1)`.
6. The above backward moving can be done at most `2 * log_{2}(k)` times. Since we've merged adjacent ones, every two adjacent elements have a product of at least `2`.
7. So the total time complexity would be `n * 2 * log_{2}(k)`.
