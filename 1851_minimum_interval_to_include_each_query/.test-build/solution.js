...
function minInterval(intervals, queries) {
    ...
    const pop = () => {
        ...
        if (heap.length) {
            ...
            while (true) {
                let t = i;
                const l = i * 2 + 1, r = l + 1;
                if (l < heap.length && (heap[l][0] < heap[t][0] || (heap[l][0] === heap[t][0] && heap[l][1] < heap[t][1])))
                    t = l;
                if (r < heap.length && (heap[r][0] < heap[t][0] || (heap[r][0] === heap[t][0] && heap[r][1] < heap[t][1])))
                    t = r;
                if (t === i)
                    break;
    ...
    for (const [query, queryIdx] of indexed) {
        ...
        while (heap.length && heap[0][1] < query)
            pop();
        if (heap.length)
            answer[queryIdx] = heap[0][0];
    }
    return answer;
}
