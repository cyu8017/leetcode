...
You are given an integer `n`. There is an **undirected** graph with `n` nodes, numbered from `0` to `n - 1`. You are given a 2D integer array `edges` where `edges[i] = [a_{i}, b_{i}]` denotes that there exists an **undirected** edge connecting nodes `a_{i}` and `b_{i}`.
