...
You are also given another array `queries` where `queries[i] = [a_{i}, b_{i}]`. On the `i^{th}` query, Alice is in building `a_{i}` while Bob is in building `b_{i}`.
