...
You are given an **undirected weighted** **connected** graph containing `n` nodes labeled from `0` to `n - 1`, and an integer array `edges` where `edges[i] = [a_{i}, b_{i}, w_{i}]` indicates that there is an edge between nodes `a_{i}` and `b_{i}` with weight `w_{i}`.
