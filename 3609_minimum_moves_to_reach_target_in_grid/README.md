...
1. Work backwards from `(tx, ty)` to `(sx, sy)`, undoing one move at each step.
...
3. Count these undo-steps until you hit `(sx, sy)` (return the count), or return -1 if you drop below or get stuck.
