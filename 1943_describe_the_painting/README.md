...
There is a long and thin painting that can be represented by a number line. The painting was painted with multiple overlapping segments where each segment was painted with a **unique** color. You are given a 2D integer array `segments`, where `segments[i] = [start_{i}, end_{i}, color_{i}]` represents the **half-closed segment** `[start_{i}, end_{i})` with `color_{i}` as the color.
...
- `1 <= segments.length <= 2 * 10^{4}`
	...
	- `1 <= start_{i} < end_{i} <= 10^{5}`

	- `1 <= color_{i} <= 10^{9}`

	- Each `color_{i}` is distinct.
