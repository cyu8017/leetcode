...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/find-the-closest-marked-node/](https://leetcode.com/problems/find-the-closest-marked-node/)
- **Premium:** Yes
- **Tags:** array, graph-theory, heap-(priority-queue), shortest-path

## Problem

You are given a positive integer `n` which is the number of nodes of a **0-indexed directed weighted** graph and a **0-indexed** **2D array** `edges` where `edges[i] = [u_{i}, v_{i}, w_{i}]` indicates that there is an edge from node `u_{i}` to node `v_{i}` with weight `w_{i}`.
...
1. Find the distance from s to all nodes.
2. You can use Dijkstra to find them.
3. Find the minimum distance between marked nodes.
