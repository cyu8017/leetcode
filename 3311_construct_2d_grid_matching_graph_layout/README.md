...
- **Difficulty:** Hard
- **LeetCode:** [https://leetcode.com/problems/construct-2d-grid-matching-graph-layout/](https://leetcode.com/problems/construct-2d-grid-matching-graph-layout/)
- **Tags:** array, hash-table, graph-theory, matrix

## Problem

You are given a 2D integer array `edges` representing an **undirected** graph having `n` nodes, where `edges[i] = [u_{i}, v_{i}]` denotes an edge between nodes `u_{i}` and `v_{i}`.
