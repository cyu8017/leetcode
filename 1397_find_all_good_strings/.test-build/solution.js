...
function findGoodStrings(n, s1, s2, evil) {
    ...
    for (let i = 1, j = 0; i < m;)
        if (evil[i] === evil[j])
            ...
            i++;
    const next = (matched, ch) => { while (matched && evil[matched] !== ch)
        matched = lps[matched - 1]; return evil[matched] === ch ? matched + 1 : 0; };
    const count = (bound) => {
        const memo = new Map();
        const dfs = (pos, matched, tight) => {
            ...
            for (let code = 97; code <= limit; code++)
                result = (result + dfs(pos + 1, next(matched, String.fromCharCode(code)), tight && code === limit)) % mod;
            if (!tight)
                memo.set(key, result);
            return result;
        };
    ...
    const decrement = (value) => { const chars = value.split(""); for (let i = chars.length - 1; i >= 0; i--) {
        if (chars[i] > "a") {
