...
You are given two integers `m` and `n` that represent the height and width of a rectangular piece of wood. You are also given a 2D integer array `prices`, where `prices[i] = [h_{i}, w_{i}, price_{i}]` indicates you can sell a rectangular piece of wood of height `h_{i}` and width `w_{i}` for `price_{i}` dollars.
