...
export class BITI {
    constructor(n_: any) { this.n = n_; this.c = new Array(n_ + 1).fill(0); }
    upd(x: any, d: any): any { for (; x <= this.n; x += x & -x) this.c[x] += d; }
    qry(x: any): any { let s = 0; for (; x > 0; x -= x & -x) s += this.c[x]; return s; }
}
export class BITL {
...
function kth(cnt: any, m: any, k: any): any {
    let idx = 0;
    ...
    return idx + 1;
}function sumSmallest(cnt: any, sum: any, uniq: any, m: any, kk: any): any {
    if (kk <= 0) return 0;
    ...
    return s;
}function lowerBound(arr: any, x: any): any {
    ...
    return lo;
}export function minimumCost(nums: any, k: any, dist: any): any {
    k--;
    const n = nums.length;
