...
class Solution {
    public int numberOfWays(int n, int m, int k, int[] source, int[] dest) {
        ...
        long same = 0, row = 0, col = 0, other = 0;
        if (sx == tx && sy == ty) same = 1;
        else if (sx == tx) row = 1;
        else if (sy == ty) col = 1;
        else other = 1;
        for (int step = 0; step < k; step++) {
            long ns = (row * (m - 1) + col * (n - 1)) % mod;
            long nr = (same + row * (m - 2) % mod + other * (n - 1) % mod) % mod;
            long nc = (same + col * (n - 2) % mod + other * (m - 1) % mod) % mod;
            long no = (row * (n - 1) + col * (m - 1) + other * (n + m - 4) % mod) % mod;
            same = ns;
            row = nr;
            col = nc;
            other = no;
