...
For each `k`, such that `1 <= k <= n`, you need to find the number of **pairs of houses** `(house_{1}, house_{2})` such that the **minimum** number of streets that need to be traveled to reach `house_{2}` from `house_{1}` is `k`.
...
2. The shortest distance between house `i` and house `j` (`j < i`) is along one of these paths:
- `i -> j`
- `i -> y---x -> j`
