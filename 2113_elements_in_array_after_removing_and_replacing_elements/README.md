...
- **Difficulty:** Medium
- **LeetCode:** [https://leetcode.com/problems/elements-in-array-after-removing-and-replacing-elements/](https://leetcode.com/problems/elements-in-array-after-removing-and-replacing-elements/)
- **Premium:** Yes
- **Tags:** array
...
You are also given a 2D integer array `queries` of size `n` where `queries[j] = [time_{j}, index_{j}]`. The answer to the `j^{th}` query is:

	- `nums[index_{j}]` if `index_{j} < nums.length` at minute `time_{j}`

	- `-1` if `index_{j} >= nums.length` at minute `time_{j}`
...
1. Try to find a pattern in how nums changes.
2. Let m be the original length of nums. If time_i / m (integer division) is even, then nums is at its original size or decreasing in size. If it is odd, then it is empty, or increasing in size.
3. time_i % m can be used to find how many elements are in nums at minute time_i.
