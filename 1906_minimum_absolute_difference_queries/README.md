...
You are given an integer array `nums` and the array `queries` where `queries[i] = [l_{i}, r_{i}]`. For each query `i`, compute the **minimum absolute difference** of the **subarray** `nums[l_{i}...r_{i}]` containing the elements of `nums` between the **0-based** indices `l_{i}` and `r_{i}` (**inclusive**).
